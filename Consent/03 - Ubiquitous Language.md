...
| **Build the jump-page URL** | `JumpPageUrlService` `HF/AppCommon/.../compliance/JumpPageUrlService.java` | Builds `profileKey/userKey[/meetingKey]` URL — **used by [[Call Scheduling/03 - Ubiquitous Language|Call Scheduling]]** for the consent link. |
...
- **Upstream — Call Scheduling:** consumes **`call-scheduling-updated`** (`CallSchedulingUpdated`) to know a call was scheduled/cancelled and schedule the consent email. See [[Call Scheduling/02 - Entry Points (Inbound & Outbound)]] — this is the exact downstream hand-off documented there.
...
- [[Call Scheduling/03 - Ubiquitous Language]] — the upstream domain that hands off `call-scheduling-updated`
