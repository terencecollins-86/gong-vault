...
> 2. **It's the inbound counterpart of [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/CALL-PROCESSING-INBOUND]].** The Supervisor *sends* calls to processing on `dialer-calls-updates`; processing reports *back* here on the `CALL_PROCESSOR` cluster. Don't confuse the two directions.
