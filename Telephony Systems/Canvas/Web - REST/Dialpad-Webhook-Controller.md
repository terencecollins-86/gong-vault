...
| **Callers** | Internal Feign / `DialpadTroubleshooter`; see [[Call Scheduling/Canvas/Telephony Systems/Providers/Dialpad]] |
...
- Postman: `Other Troubleshooters → Dialpad Troubleshooter`. See [[Call Scheduling/Canvas/Telephony Systems/Providers/Dialpad]] and [[Entrypoints Within the Telephony System]].
...
| Dialpad call events not arriving for ingestion | Subscription/websocket missing — `listCallEvents` + `listWebsockets`; (re)create via `createCallEvent`. Then trace ingest via [[Call Scheduling/Canvas/Telephony Systems/Providers/Dialpad]] / [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/CALL-PROCESSING-INBOUND]]. |
