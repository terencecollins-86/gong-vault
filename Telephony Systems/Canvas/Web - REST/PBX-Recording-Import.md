...
Step `:195` → if FF true, into `sendDialerCall(...)` @198 → then the event lands on `dialer-calls-updates` (see [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]]).
