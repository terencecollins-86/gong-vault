...
> The other two Gong Connect services — **GongConnectWebApi** and **GongConnectTasks** — also produce `TelephonyCallEvent`s to **`gong-connect-dialer-events`**, the same topic as [[Consent/Canvas 1/Telephony Systems/Inbound Producers/GongConnectWebhooksServer]]. They feed the **same** Supervisor consumer (`TelephonyCallEventConsumer`). If only one of the three producers misbehaves, push-path calls from that source go missing while others flow — so when triaging "some calls missing," you can't tell the three apart from the consumer side alone; correlate by `providerName`/company in the payload.
>
...
> Related: [[Call Scheduling/Canvas/Telephony Systems/Core/IngesterTelephonySystemsSupervisor]] · [[Call Scheduling/Canvas/Telephony Systems/Inbound Topics/GONG-CONNECT-DIALER-EVENTS]] · [[Consent/Canvas 1/Telephony Systems/Inbound Producers/GongConnectWebhooksServer]] · [[Entrypoints Within the Telephony System]]
