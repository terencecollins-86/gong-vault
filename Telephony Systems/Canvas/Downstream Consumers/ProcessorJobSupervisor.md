...
| **Topic our code actually produces** | `dialer-calls-updates`, cluster `SYNC_COMMUNICATIONS` (see [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]]) |
...
| "Should be on `call-processing-inbound`" confusion | It isn't — Supervisor emits `dialer-calls-updates`; an intermediary routes to `call-processing-inbound`. Trace the topic the code actually uses. See [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/CALL-PROCESSING-INBOUND]]. |

> Related: [[Call Scheduling/Canvas/Telephony Systems/Core/IngesterTelephonySystemsSupervisor]] · [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]] · [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/CALL-PROCESSING-INBOUND]]
