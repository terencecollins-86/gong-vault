...
> 2. The descriptor declares `WRITE` to `call-processing-inbound` (`IngesterTelephonySystemsSupervisor.gong-app-descriptor.yaml:84/86`) but the real outbound hand-off goes on **`dialer-calls-updates`** (`SYNC_COMMUNICATIONS`, `:112`). Chase *that* topic — see [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]] / [[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/CALL-PROCESSING-INBOUND]].
...
|---|---|---|
| `DialpadController` (+ advice, troubleshooter) | [[Call Scheduling/Canvas/Telephony Systems/Web - REST/Dialpad-Webhook-Controller]] | internal Feign / `DialpadTroubleshooter` · [[Call Scheduling/Canvas/Telephony Systems/Providers/Dialpad]] |
| `PbxRecordingImportController` | [[Call Scheduling/Canvas/Telephony Systems/Web - REST/PBX-Recording-Import]] | PublicApiServer / WebFrontEnd |
| `TelephonySystemsCallDataController` | [[Call Scheduling/Canvas/Telephony Systems/Web - REST/CallData-Controller]] | CallPipelineExecutor |
| `TelephonySystemsCallDataProviderController` | [[Call Scheduling/Canvas/Telephony Systems/Web - REST/CallData-Provider-Controller]] | Orchestrator |
...
| Calls ingested but never processed | Is `SEND_DIALER_CALL_CREATION_EVENT` on for the company? Then Coralogix for the GDM-send warn; lag on `dialer-calls-updates` ([[Call Scheduling/Canvas/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]]). |
