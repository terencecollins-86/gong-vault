...
| **Downstream** | [[Call Scheduling/Canvas/Telephony Systems/Downstream Consumers/CommunicationsSyncServer]] (consumes `dialer-calls-updates`) |
...
- `UploadedCallEventService.java:63` — dialer call (the conference-call sibling at `:44` goes to [[Consent/Canvas 1/Telephony Systems/Outbound Topics/WEBCONFERENCE-CALL-EVENTS]] instead)
...
| "Should be on `call-processing-inbound`" confusion | It isn't — Supervisor emits `dialer-calls-updates`; the `call-processing-inbound*` descriptor grants have no producer. See [[Consent/Canvas 1/Telephony Systems/Outbound Topics/CALL-PROCESSING-INBOUND]] / [[Consent/Canvas 1/Telephony Systems/Outbound Topics/CALL-PROCESSING-INBOUND-LOW-PRIORITY]]. |
