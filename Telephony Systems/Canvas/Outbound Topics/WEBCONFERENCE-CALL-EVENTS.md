...
> The web-conference sibling of [[Consent/Canvas 1/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]] — hand-off of an ingested **conference call** to the downstream comms-sync / call-processing pipeline. If this stops, **conference calls land in Gong but never get forwarded for processing.**
>
...
| **Downstream** | [[Call Scheduling/Canvas/Telephony Systems/Downstream Consumers/CommunicationsSyncServer]] (consumes `webconference-call-events`) |
...
- `UploadedCallEventService.java:44` — `gdmCallEventSender.sendWebConfCall(...)` (the dialer sibling at `:63` goes to [[Consent/Canvas 1/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]] instead)
