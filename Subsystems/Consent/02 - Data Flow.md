...
| Consumer `:line`                          | Module                    | Cluster             | Topic (literal)                   | Payload                            | Why it exists                                                                                                                                                                                                                                      |
| ----------------------------------------- | ------------------------- | ------------------- | --------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AuditMeetingConsentConsumer:26`          | RecordingConsentTasks     | `RECORDING_CONSENT` | `audit-meeting-consent`           | `JumpPageInteractionEvent`         | A participant just accepted or declined on the jump page. Writes the compliance audit trail (`jump_page_session` + `jump_page_interaction`) so legal/support have a durable record of every consent decision.                                      |
| `AuditStopRecordingConsumer:25`           | RecordingConsentTasks     | `RECORDING_CONSENT` | `audit-stop-recording`            | `StopRecordingEvent`               | Recording was stopped mid-call (e.g. participant opted out late or a compliance rule triggered). Persists a `stop_recording_audit` row so there is an immutable log of when and why recording was halted.                                          |
| `CalendarUpdatesForConsentConsumer:24`    | RecordingConsentTasks     | `RECORDING_CONSENT` | `calendar-updates-for-consent`    | `CalendarUpdateEvent`              | A calendar event was created, updated, or cancelled. Keeps Consent's `calendar_event` mirror in sync so the subsystem knows which upcoming meetings require a jump page or consent email.                                                          |
| `ConsentEmailPageInteractionConsumer:22`  | RecordingConsentTasks     | `RECORDING_CONSENT` | `consent-email-page-interaction`  | `ConsentEmailPageInteractionEvent` | A participant clicked a link in a pre-call consent email and interacted with the email landing page. Records the interaction so Consent knows whether the email-based consent path completed before the call starts.                               |
| `ResetConsentRedisForCompanyConsumer:17`  | RecordingConsentTasks     | `RECORDING_CONSENT` | `reset-consent-redis-for-company` | `ResetConsentRedisForCompanyEvent` | A company's DCP settings just changed. Evicts the company's stale jump-page data from Redis so the next page render uses the new DCP profile instead of the cached one.                                                                            |
| `ChangeRequestExecutorConsumer:24`        | DcpChangeManager          | `DATA_CAPTURE`      | `change-request-executor`         | `DcpChangeRequestEvent`            | A DCP settings change was submitted at the company level. Entry point for the `ChangeRequestLifecycle` state machine — kicks off the fan-out that propagates the change to all affected users.                                                     |
| `SingleUserChangeExecutorConsumer:23`     | DcpChangeManager          | `DATA_CAPTURE`      | `single-user-change-executor`     | `DcpChangeRequestEvent`            | The batch fan-out has assigned one user's portion of a DCP change. Applies the concrete change actions (cancel non-compliant calls, update consent email settings, sync PMI meeting URL) for that single user.                                     |
| `BatchUsersChangeExecutorConsumer:23`     | DcpChangeManager          | `DATA_CAPTURE`      | `batch-users-change-executor`     | `DcpChangeRequestEvent`            | A new company-level DCP change request arrived. Fans the change out to all affected users in parallel by producing one `single-user-change-executor` message per user. Decouples scale of the company's user base from the initial change request. |
| `SingleUserChangeRequestDoneConsumer:23`  | DcpChangeManager          | `DATA_CAPTURE`      | `single-user-change-request-done` | `DcpUserChangeRequestDoneEvent`    | One user's DCP change completed. Drives the state machine forward — when all users are done, the change request closes and Redis is flushed via `reset-consent-redis-for-company`.                                                                 |
| `RecordingConsentPurgeCompanyConsumer:23` | RecordingConsentApiServer | `OPERATIONAL_V1`    | `purge-company`                   | `PurgeCompany` (concurrency 1)     | A company is being deleted from Gong (GDPR right-to-erasure or offboarding). Purges all consent data owned by that company — concurrency 1 to avoid races during a destructive operation.                                                          |
