...
| ----------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| **Data Capture Profile (DCP)**      | `DcpJumpPageSettings` / `DcpJumpPageUrlSettings`                                                  | `profileKey` (+ company)                 | Company-level recording-consent configuration — what the jump page enforces, per provider. Holds a `List<DcpJumpPageSettingsProvider>` — one entry per covered provider, each with independent `linkType` and `recordingOptOut`. See [[Meeting Providers & Multi-Provider DCP]]. | gong-clients dto; used `MeetingFrontEnd/.../JumpPageController.java:248`                                        |
| **Jump page** ("consent page")      | rendered by `JumpPageController`                                                                  | `profileKey/userKey[/meetingKey]`        | The participant-facing consent artifact. 2 URL segments = **PMI / static**; 3 = **dynamic / one-time meeting**. Multi-provider profiles append `?provider=<code>` to discriminate the provider at render time. | `GDC/MeetingFrontEnd/.../controller/JumpPageController.java:286`                                                |
...
| **User settings**                   | `recording_consent_settings.user_settings` (DAO `UserSettingsDao`)                                | `(company, userId)`                      | Per-user default meeting provider (`default_meeting_provider` column). Written by `saveUserProviderDefault`; cleared by `ResetUserDefaultProviderAction` when a provider is removed from the DCP. | `GDC/RecordingConsentTasks/.../service/UserSettingsDao.java:18`                                                 |
...
| **JumpPageLinkType** | `PMI`, `DYNAMIC`, `ONLY_DYNAMIC` | The link type for a provider entry within a DCP. `isPmiEnabled(provider)` / `isDynamicEnabled()` used at URL-build time. | `HF/AppCommon/.../datacaptureprofile/JumpPageLinkType.java` |
| **JumpPageRecordingOptOut** | `EXPLICIT`, `IMPLICIT` | `EXPLICIT` = Deny button shown on consent page. `IMPLICIT` = no deny button; joining = implicit consent. Per-provider within a DCP. | `HF/AppCommon/.../datacaptureprofile/JumpPageRecordingOptOut.java` |
...
**Multi-provider URL discriminator:** `?provider=<code>` query param, appended by
`MultipleProviderJumpPageUrlService#createMultiProviderUsersUrl`. Parsed back at render time by
`#extractProviderFromUrl`. See [[Meeting Providers & Multi-Provider DCP]].
...
- [[Meeting Providers & Multi-Provider DCP]] — full provider enum list, per-provider DCP settings, `?provider=` URL discriminator, user provider default lifecycle
