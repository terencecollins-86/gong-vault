...
`NO_RESPONSE` → immediate return, no action (call records). 
`ACCEPTED` → no action (call records). 
`DENIED` → `callSchedulerClient.cancelScheduledCallByConsentEmail` → `SkipCode.CANCEL_BY_COMPLIANCE_EMAIL`. 

Silence is treated as no objection. 
The email is a **notification**, not a gate.
