...
> 2. The descriptor declares `WRITE` to `call-processing-inbound` (`IngesterTelephonySystemsSupervisor.gong-app-descriptor.yaml:84/86`) but the real outbound hand-off goes on **`dialer-calls-updates`** (`SYNC_COMMUNICATIONS`, `:112`). Chase *that* topic — see [[Subsystems/Call Scheduling/Canvas/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]] / [[Subsystems/Call Scheduling/Canvas/Telephony Systems/Outbound Topics/CALL-PROCESSING-INBOUND]].
...
|---|---|---|
| `DialpadController` (+ advice, troubleshooter) | [[Subsystems/Call Scheduling/Canvas/Telephony Systems/Web - REST/Dialpad-Webhook-Controller]] | internal Feign / `DialpadTroubleshooter` · [[Subsystems/Call Scheduling/Canvas/Telephony Systems/Providers/Dialpad]] |
| `PbxRecordingImportController` | [[Subsystems/Call Scheduling/Canvas/Telephony Systems/Web - REST/PBX-Recording-Import]] | PublicApiServer / WebFrontEnd |
| `TelephonySystemsCallDataController` | [[Subsystems/Call Scheduling/Canvas/Telephony Systems/Web - REST/CallData-Controller]] | CallPipelineExecutor |
| `TelephonySystemsCallDataProviderController` | [[Subsystems/Call Scheduling/Canvas/Telephony Systems/Web - REST/CallData-Provider-Controller]] | Orchestrator |
...
Swagger (prod): `https://ingestertelephonysystemssupervisor-vip.prod.gongio.net/swagger-ui/index.html`. Full pattern: [[Work/Architecture/Troubleshoot Endpoints]] · [[06 - Runbook & Troubleshooting]].
...
| Calls ingested but never processed | Is `SEND_DIALER_CALL_CREATION_EVENT` on for the company? Then Coralogix for the GDM-send warn; lag on `dialer-calls-updates` ([[Subsystems/Call Scheduling/Canvas/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]]). |
