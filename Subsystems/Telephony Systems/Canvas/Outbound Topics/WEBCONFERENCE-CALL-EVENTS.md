...
> The web-conference sibling of [[Subsystems/Consent/Canvas 1/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]] — hand-off of an ingested **conference call** to the downstream comms-sync / call-processing pipeline. If this stops, **conference calls land in Gong but never get forwarded for processing.**
>
...
| **Downstream** | [[Subsystems/Call Scheduling/Canvas/Telephony Systems/Downstream Consumers/CommunicationsSyncServer]] (consumes `webconference-call-events`) |
...
- `UploadedCallEventService.java:44` — `gdmCallEventSender.sendWebConfCall(...)` (the dialer sibling at `:63` goes to [[Subsystems/Consent/Canvas 1/Telephony Systems/Outbound Topics/DIALER-CALLS-UPDATES]] instead)
